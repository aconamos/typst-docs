...
=== Polar Integration
$ A = integral d A = 1/2 integral r^2 d theta $
...
=== Power Series
$A_n$ is the alternating term, $(-1)^n$
#table(
  columns: (auto, auto, auto),
  inset: 10pt,
  align: horizon,
  table.header(
    [*$f(x)$*], [*MacLaurin series*], [*Radius of Convergene*]
  ),
  $e^x$, $ sum_(n=0)^oo x^n / n! $, $oo$, 
  $sin(x)$, $ sum_(n=0)^oo A_n dot (x^(2n+1))/(2n+1)! $, $oo$,
  $cos(x)$, $ sum_(n=0)^oo A_n dot (x^(2n))/(2n)! $, $oo$,
  $1/(1-x)$, $ sum_(n=0)^oo x^n $, $1$
)

=== Exam Brain
- Taylor Series of $f$, centered at $a$: $ sum_(n=0)^oo (f^(n)(a) dot (x-a)^n)/(n!) $
